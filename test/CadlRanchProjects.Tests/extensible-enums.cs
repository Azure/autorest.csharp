...
namespace CadlRanchProjects.Tests
{
    public class extensible_enums : CadlRanchTestBase
    {
        ...
        {
            Response response = await new StringClient(host, null).GetKnownValueAsync();
        ...
        {
            Response response = await new StringClient(host, null).GetUnknownValueAsync();
            JsonData result = JsonData.FromBytes(response.Content.ToMemory());
        ...
        {
            Response response = await new StringClient(host, null).PutKnownValueAsync(RequestContent.Create(new JsonData(DaysOfWeekExtensibleEnum.Monday.ToString())));
        ...
        {
            Response response = await new StringClient(host, null).PutUnknownValueAsync(RequestContent.Create(new JsonData(new DaysOfWeekExtensibleEnum("Weekend").ToString())));
            Assert.AreEqual(204, response.Status);
        });
    }
}
