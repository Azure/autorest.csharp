    ...
    public class Bug1763 : BugTest
    {
        ...
        public async Task VerifyUnderscoredClassMembers()
        {
            using (var fileSystem = GenerateCodeForTestFromSpec())
            {
                ...
                var regex = new Regex(Regex.Escape("public Pet(int _id)") + @"[^}]+");
        ...
        [Fact]
        public async Task VerifyUnderscoredClassMembersOptionalParams()
        {
            using (var fileSystem = GenerateCodeForTestFromSpec(GetType().Name, null, settings => settings.CustomSettings.Add("HaveOptionalPropertiesOnConstructors", true)))
            {
                // throw new System.Exception(fileSystem.ReadAllText(Path.Combine("Models", "Pet.cs")));
                var result = await Compile(fileSystem);
