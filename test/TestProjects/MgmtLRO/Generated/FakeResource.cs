    ...
    public partial class FakeResource : ArmResource
    {
        ...
        public virtual async Task<ArmOperation<FakePostResult>> DoSomethingLROAsync(WaitUntil waitUntil, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var response = await _fakeRestClient.DoSomethingLROAsync(message, cancellationToken).ConfigureAwait(false);
                var operation = new MgmtLROArmOperation<FakePostResult>(new FakePostResultOperationSource(), _fakeClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    await operation.WaitForCompletionAsync(cancellationToken).ConfigureAwait(false);
                return operation;
            }
        ...
        public virtual ArmOperation<FakePostResult> DoSomethingLRO(WaitUntil waitUntil, CancellationToken cancellationToken = default)
        {
            ...
            {
                using var message = _fakeRestClient.CreateDoSomethingLRORequest(Id.SubscriptionId, Id.ResourceGroupName, Id.Name);
                var response = _fakeRestClient.DoSomethingLRO(message, cancellationToken);
                var operation = new MgmtLROArmOperation<FakePostResult>(new FakePostResultOperationSource(), _fakeClientDiagnostics, Pipeline, message.Request, response, OperationFinalStateVia.Location);
                if (waitUntil == WaitUntil.Completed)
                    operation.WaitForCompletion(cancellationToken);
                return operation;
