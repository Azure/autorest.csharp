...
namespace MgmtExpandResourceTypes
{
...

    {
        ...
        public virtual RecordSetDeleteOperation Delete(string relativeRecordSetName, string ifMatch = null, bool waitForCompletion = true, CancellationToken cancellationToken = default)
        {
            ...
            using var scope = _clientDiagnostics.CreateScope("RecordSetSoaCollection.Delete");
            scope.Start();
            try
            {
                var response = _recordSetsRestClient.Delete(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, "SOA".ToRecordType(), relativeRecordSetName, ifMatch, cancellationToken);
        ...
        public async virtual Task<RecordSetDeleteOperation> DeleteAsync(string relativeRecordSetName, string ifMatch = null, bool waitForCompletion = true, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var response = await _recordSetsRestClient.DeleteAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, "SOA".ToRecordType(), relativeRecordSetName, ifMatch, cancellationToken).ConfigureAwait(false);
                var operation = new RecordSetDeleteOperation(response);
                if (waitForCompletion)
