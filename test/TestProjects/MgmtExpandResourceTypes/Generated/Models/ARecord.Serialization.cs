...
namespace MgmtExpandResourceTypes.Models
{
    ...
    {
        void IUtf8JsonSerializable.Write(Utf8JsonWriter writer) => ((IJsonModel<ARecord>)this).Write(writer, new ModelReaderWriterOptions("W"));

        void IJsonModel<ARecord>.Write(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            var format = options.Format == "W" ? ((IPersistableModel<ARecord>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new InvalidOperationException($"The model {nameof(ARecord)} does not support '{format}' format.");
            }
        ...
        BinaryData IPersistableModel<ARecord>.Write(ModelReaderWriterOptions options)
        {
            ...
            switch (format)
            {
                ...
                default:
                    throw new InvalidOperationException($"The model {nameof(ARecord)} does not support '{options.Format}' format.");
            }
        }
        ...
        string IPersistableModel<ARecord>.GetFormatFromOptions(ModelReaderWriterOptions options) => "J";
    }
}
