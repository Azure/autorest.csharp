        ...
        public void GetShouldMatchResource()
        {
            ...
            foreach (var collection in FindAllCollections())
            {
                ...
                if (!ListExceptionCollections.Contains(collection))
                    VerifyMethodReturnType(collection, resource, collection.GetMethods().First(m => m.Name == "GetAll" && !m.GetParameters().Any(p => !p.IsOptional && p.ParameterType.Name.EndsWith(m.Name + "GetAllOptions"))));
