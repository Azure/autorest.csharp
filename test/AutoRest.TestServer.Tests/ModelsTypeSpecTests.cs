...
namespace AutoRest.TestServer.Tests
{
    ...
    public class ModelsTypeSpecTests
    {
        ...
        public void Test()
        {
            Payload.JsonMergePatch.Models.BaseModel model = new ExtendedModel();
            var type = typeof(ExtendedModel);
            var methodInfo = type.GetMethods();
            //ModelReaderWriter.Write(model);
            if (model is IJsonModel<BaseModel> jsonModel)
            {
                var content = new Utf8JsonRequestContent();
                jsonModel.Write(content.JsonWriter, new ModelReaderWriterOptions("J"));
            }
        }
