...
namespace AutoRest.TestServer.Tests
{
    public class BodyStringTest : TestServerLowLevelTestBase
    {
        ...
        {
            var result = await new StringClient(Key, host).GetMbcsAsync(new());
        ...
        {
            // Empty response body
            var result = await new StringClient(Key, host).GetNotProvidedAsync(new());
        ...
        {
            var result = await new StringClient(Key, host).GetBase64EncodedAsync(new());
            var responseBody = JsonData.FromBytes(result.Content.ToMemory());
        ...
        {
            // Empty response body
            var result = await new StringClient(Key, host).GetNullAsync(new());
            Assert.Zero (result.Content.ToMemory().Length);
        });
        ...
        {
            var result = await new StringClient(Key, host).GetEmptyAsync(new());
            var responseBody = JsonData.FromBytes(result.Content.ToMemory());
        ...
        {
            var result = await new StringClient(Key, host).GetWhitespaceAsync(new());
            var responseBody = JsonData.FromBytes(result.Content.ToMemory());
        ...
        {
            var result = await new EnumClient(Key, host).GetReferencedAsync(new());
            var responseBody = JsonData.FromBytes(result.Content.ToMemory());
        ...
        {
            var result = await new EnumClient(Key, host).GetReferencedConstantAsync(new());
            var responseBody = JsonData.FromBytes(result.Content.ToMemory());
        ...
        {
            var result = await new EnumClient(Key, host).GetNotExpandableAsync(new());
            var responseBody = JsonData.FromBytes(result.Content.ToMemory());
