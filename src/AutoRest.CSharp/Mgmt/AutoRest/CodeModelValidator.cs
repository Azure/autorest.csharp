        ...
        public static void Validate(CodeModel model) {
            foreach (var group in model.OperationGroups)
            {
                if (string.IsNullOrEmpty(group.Key)) {
                    ErrorHelpers.ThrowError("Malformed 'operationId' detected. Check swagger definition and make sure every 'operatoinId' follows form of 'Noun_Verb'." +
                        " For details, refer to: https://github.com/Azure/azure-rest-api-specs/blob/master/documentation/openapi-authoring-automated-guidelines.md#r1001\n" +
