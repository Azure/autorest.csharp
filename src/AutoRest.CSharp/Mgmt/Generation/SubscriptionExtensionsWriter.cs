...
namespace AutoRest.CSharp.Mgmt.Generation
{
    internal class SubscriptionExtensionsWriter : MgmtExtensionWriter
    {
        ...
        private void WriteGetAllResourcesAsGenericMethod(Resource resource, bool async)
        {
            ...
            using (_writer.Scope($"public static {responseType} {CreateMethodName($"Get{resource.Type.Name.ToPlural()}AsGenericResources", async)}(this {typeof(Subscription)} subscription, {typeof(string)} filter, {typeof(string)} expand, {typeof(int?)} top, {typeof(CancellationToken)} cancellationToken = default)"))
            {
