        ...
        public void Write()
        {
            using (_writer.Namespace(MgmtContext.Context.DefaultNamespace))
            {
                ...
                using (_writer.Scope())
                {
                    ...
                    using (_writer.Scope($"internal {_name}({_operationSourceString}{typeof(ClientDiagnostics)} clientDiagnostics, {typeof(HttpPipeline)} pipeline, {typeof(HttpMessage)} message, {typeof(Response)} response, {typeof(OperationFinalStateVia)} finalStateVia, {typeof(string)} userAgent)"))
                    {
                        _writer.Line($"_operation = ArmOperationHelpers.CreateOperation({_sourceString}clientDiagnostics, pipeline, message, response, finalStateVia, \"{_name}\", userAgent);");
                    }
                    _writer.Line();
