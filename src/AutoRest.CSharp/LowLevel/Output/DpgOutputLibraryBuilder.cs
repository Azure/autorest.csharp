...
namespace AutoRest.CSharp.Output.Models
{
    internal class DpgOutputLibraryBuilder
    {
        ...
        public DpgOutputLibraryBuilder(InputNamespace rootNamespace, SourceInputModel? sourceInputModel)
        {
            ...
            rootNamespace = ExampleMockValueBuilder.Visit(rootNamespace);
        ...
        private void CreateModels(IDictionary<InputModelType, ModelTypeProvider> models, TypeFactory typeFactory)
        {
            ...
            foreach (var model in _rootNamespace.Models)
            {
                ...
                models.Add(model, new ModelTypeProvider(model, TypeProvider.GetDefaultModelNamespace(null, _defaultNamespace), _sourceInputModel, typeFactory, model.DerivedModels.ToArray(), defaultDerivedType));
            }
        }
        ...
        {
            var topLevelClients = CreateClients(topLevelClientInfos, clientOptions, null, typeFactory);
        ...
        private IEnumerable<DpgClient> CreateClients(IEnumerable<ClientInfo> clientInfos, ClientOptionsTypeProvider clientOptions, DpgClient? parentClient, TypeFactory typeFactory)
        {
            foreach (var clientInfo in clientInfos)
            {
                ...
                subClients.AddRange(CreateClients(clientInfo.Children, clientOptions, client, typeFactory));
