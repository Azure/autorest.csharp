    ...
    internal class DpgOutputLibraryBuilder
    {
        ...
        private IReadOnlyList<ClientInfo> SetHierarchy(IReadOnlyDictionary<string, ClientInfo> clientInfosByName)
        {
            ...
            if (topLevelClientInfo == null)
            {
                ...
                var clientNamespace = Configuration.Namespace;
                var clientParameters = topLevelClients.SelectMany(c => c.ClientParameters.Concat(c.Operations.SelectMany(op => op.Parameters).Where(p => p.Kind == InputOperationParameterKind.Client).DistinctBy(p => p.Name)).Where(p => !p.IsRequired || p.IsApiVersion || p.IsEndpoint)).Distinct().ToArray();
