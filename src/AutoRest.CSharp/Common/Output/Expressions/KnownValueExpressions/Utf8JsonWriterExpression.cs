...
namespace AutoRest.CSharp.Common.Output.Expressions.KnownValueExpressions
{
    internal sealed record Utf8JsonWriterExpression(ValueExpression Untyped) : TypedValueExpression<Utf8JsonWriter>(Untyped)
    {
        ...
        public MethodBodyStatement WriteBinaryData(BinaryDataExpression value)
            => new IfElsePreprocessorDirective
                (
                    ...
                    {
                        JsonSerializerExpression.Serialize(this, new JsonDocumentExpression(jsonDocumentVar).RootElement)
                    }
                );
    }
}
