    ...
    internal abstract class RestClient : TypeProvider
    {
        ...
        private IEnumerable<RestClientMethod> BuildAllMethods()
        {
            ...
            foreach (var operation in InputClient.Operations)
            {
                ...
                if (GetNextOperationMethod(operation) is { } nextOperationMethod && operation is {Paging.NextLinkOperation: null, Paging.SelfNextLink: false})
                {
        ...
        protected Dictionary<InputOperation, RestClientMethod> EnsureGetNextPageMethods()
        {
            ...
            foreach (var operation in InputClient.Operations)
            {
                ...
                {
                    nextMethod = GetOperationMethod(operation);
                }
                else if (paging.NextLinkOperation != null)
                {
