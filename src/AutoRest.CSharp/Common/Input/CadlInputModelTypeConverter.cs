    ...
    internal sealed class CadlInputModelTypeConverter : JsonConverter<InputModelType>
    {
        ...
        public static InputModelType CreateModelType(ref Utf8JsonReader reader, string? id, string? name, JsonSerializerOptions options, ReferenceResolver resolver)
        {
            ...
            while (reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                {
                    model = CreateInputModelTypeInstance(id, name, ns, accessibility, description, usageString, baseModel, properties, resolver);
                    reader.Read();
            ...
            return model ?? CreateInputModelTypeInstance(id, name, ns, accessibility, description, usageString, baseModel, properties, resolver);
        }

        private static InputModelType CreateInputModelTypeInstance(string? id, string? name, string? ns, string? accessibility, string? description, string? usageString, InputModelType? baseModel, List<InputModelProperty> properties, ReferenceResolver resolver)
        {
            ...
            var model = new InputModelType(name, ns, accessibility, description, usage, properties, baseModel, new List<InputModelType>(), null);
            if (id != null)
            {
