...
{
    internal sealed class TypeSpecInputTypeConverter : JsonConverter<IType>
    {
        ...
        public override IType? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            return reader.ReadReferenceAndResolve<IType>(_referenceHandler.CurrentResolver) ?? CreateObject(ref reader, options);
        }

        public override void Write(Utf8JsonWriter writer, IType value, JsonSerializerOptions options)
            => throw new NotSupportedException("Writing not supported");

        private IType CreateObject(ref Utf8JsonReader reader, JsonSerializerOptions options)
        {
            string? id = null;
            string? kind = null;
            string? name = null;
            ...
            while (reader.TokenType != JsonTokenType.EndObject)
            {
                var isIdOrNameOrKind = reader.TryReadReferenceId(ref isFirstProperty, ref id)
                    ...
                    || reader.TryReadString(nameof(IType.Name), ref name);
        ...
        private IType CreateDerivedType(ref Utf8JsonReader reader, string? id, string? kind, string? name, JsonSerializerOptions options) => kind switch
        {
        ...
        public static IPrimitiveType ReadPrimitiveType(ref Utf8JsonReader reader, string? id, string? name, ReferenceResolver resolver)
        {
            ...
            while (reader.TokenType != JsonTokenType.EndObject)
            {
                var isKnownProperty = reader.TryReadReferenceId(ref isFirstProperty, ref id)
                    || reader.TryReadBoolean(nameof(IPrimitiveType.IsNullable), ref isNullable)
                    || reader.TryReadString(nameof(IPrimitiveType.Name), ref name); // the primitive kind in the json is represented by the property `Name`.
        ...
        public static IPrimitiveType CreatePrimitiveType(string? inputTypeKindString, bool isNullable)
        {
        ...
        private static IIntrinsicType ReadIntrinsicType(ref Utf8JsonReader reader, string? id, string? name, ReferenceResolver resolver)
        {
            var isFirstProperty = id == null;
            while (reader.TokenType != JsonTokenType.EndObject)
            {
                var isKnownProperty = reader.TryReadReferenceId(ref isFirstProperty, ref id)
                    || reader.TryReadString(nameof(IIntrinsicType.Kind), ref name); // the InputIntrinsicType kind in the json is represented by the property `Name`.
        ...
        private static IIntrinsicType CreateIntrinsicType(string? inputTypeKindString)
        {
            Argument.AssertNotNull(inputTypeKindString, nameof(inputTypeKindString));
