...
namespace AutoRest.CSharp.Common.Input
{
    internal class InputTypeUsageProvider
    {
        ...
        private readonly CachedDictionary<InputType, InputModelTypeUsage> _usages;

        public InputTypeUsageProvider(InputNamespace inputNamespace)
        {
            _inputNamespace = inputNamespace;
            _usages = new CachedDictionary<InputType, InputModelTypeUsage>(EnsureUsages);
        }

        private Dictionary<InputType, InputModelTypeUsage> EnsureUsages()
        {
            var usages = new Dictionary<InputType, InputModelTypeUsage>();
            foreach (var inputModel in _inputNamespace.Models)
            {
                Apply(usages, inputModel, inputModel.Usage, true);
            }
        ...
        {
            _usages.TryGetValue(inputType, out var usage);
            return usage;
        }
    }
}
