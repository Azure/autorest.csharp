...
namespace AutoRest.CSharp.Generation.Types
{
    internal class TypeFactory
    {
        ...
        public CSharpType CreateType(IType inputType) => inputType switch
        {
            ILiteralType literalType => CSharpType.FromLiteral(CreateType(literalType.LiteralValueType), literalType.Value),
            IUnionType unionType => CSharpType.FromUnion(unionType.UnionItemTypes.Select(CreateType).ToArray(), unionType.IsNullable),
            IListType listType => new CSharpType(typeof(IList<>), listType.IsNullable, CreateType(listType.ElementType)),
            IDictionaryType dictionaryType => new CSharpType(typeof(IDictionary<,>), inputType.IsNullable, typeof(string), CreateType(dictionaryType.ValueType)),
            IEnumType enumType => _library.ResolveEnum(enumType).WithNullable(inputType.IsNullable),
            ...
            IModelType { Namespace: "Azure.Core.Foundations", Name: "Error" } => SystemObjectType.Create(AzureResponseErrorType, AzureResponseErrorType.Namespace!, null).Type,
            IModelType model => _library.ResolveModel(model).WithNullable(inputType.IsNullable),
            IPrimitiveType primitiveType => primitiveType.Kind switch
            {
                ...
                InputPrimitiveTypeKind.BytesBase64Url => Configuration.ShouldTreatBase64AsBinaryData ? new CSharpType(typeof(BinaryData), inputType.IsNullable) : new CSharpType(typeof(byte[]), inputType.IsNullable),
                ...
                InputPrimitiveTypeKind.ResourceIdentifier => new CSharpType(typeof(ResourceIdentifier), inputType.IsNullable),
            ...
            IIntrinsicType { Kind: InputIntrinsicTypeKind.Unknown } => typeof(BinaryData),
