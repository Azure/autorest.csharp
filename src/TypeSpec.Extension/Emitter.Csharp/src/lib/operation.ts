...
export function fromSdkServiceMethod(
    ...
    return {
        ...
        RequestBodyMediaType: sdkTypeToBodyMediaType(
...
function fromHttpOperationParameter(
    ...
    return {
        ...
        IsApiVersion:
            ...
            p.name.toLocaleLowerCase() === "api-version",
        IsContentType: isContentType,
        IsEndpoint: false,
        Explode:
            parameterType.Kind === "array" && format === "multi" ? true : false,
...
function loadLongRunningOperation(
    ...
    | import("../type/operation-long-running.js").OperationLongRunning
    | undefined {
    if (method.kind !== "lro") {
        return undefined;
    }
    ...
    return {
        FinalStateVia: convertLroFinalStateVia(
            method.__raw_lro_metadata.finalStateVia
        ),
        FinalResponse: {
...
function getSdkMethodResponses(
    ...
    method.operation.responses.forEach((r, range) => {
        responses.push({
            ...
            BodyType: r.type
                ? fromSdkType(r.type, sdkContext, modelMap, enumMap)
                : undefined,
            BodyMediaType: BodyMediaType.Json,
            Headers: toHttpResponseHeaders(
                r.headers,
                sdkContext,
                modelMap,
                enumMap
            ...
            IsErrorResponse:
                r.type !== undefined &&
                isErrorModel(sdkContext.program, r.type.__raw!),
            // TODO: https://github.com/Azure/typespec-azure/issues/992
            ContentTypes:
                r.contentTypes && r.contentTypes.length > 0
                    ? r.contentTypes
                    : undefined
        });
    });
...
function getParameterKind(
    ...
    if (p.kind === "body") {
        switch (p.correspondingMethodParams.length) {
            case 0:
                throw new Error(
                    `Body parameter "${p.name}" should have corresponding method parameter.`
                );
            case 1:
