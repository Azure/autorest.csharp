...
import { getInputAuth } from "./service-authentication.js";
...
export function createModel(
    ...
    function getClientName(
        ...
        if (parentClientNames.length >= 2)
            return `${parentClientNames
                .slice(parentClientNames.length - 1)
                .join("")}${clientName}`;
    ...
    function fromSdkEndpointParameter(
        ...
        const endpointVariableName = endpointExpr.substring(
            1,
            endpointExpr.length - 1
        );
