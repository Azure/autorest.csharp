...
export async function $onEmit(context: EmitContext<CSharpEmitterOptions>) {
    ...
    if (!program.compilerOptions.noEmit && !program.hasError()) {
        ...
        if (root) {
            ...
            const generatedFolder = resolvePath(
                outputFolder,
                "src",
                "Generated"
            );
            ...
            const configurations: Configuration = createConfiguration(
                options,
                namespace,
                sdkContext
            );
            ...
            const emitterPath =
                options["emitter-extension-path"] ?? import.meta.url;
            const projectRoot = findProjectRoot(
                dirname(fileURLToPath(emitterPath))
            );
            ...
            try {
                const result = await execCSharpGenerator(sdkContext, {
                    ...
                    newProject:
                        options["new-project"] || !checkFile(csProjFile),
...
export async function _validateDotNetSdk(
    ...
    try {
        ...
        return validateDotNetSdkVersionCore(
            sdkContext,
            result.stdout,
            minMajorVersion
        );
...
function validateDotNetSdkVersionCore(
    ...
    if (version) {
        ...
        const firstPart =
            dotIndex === -1 ? version : version.substring(0, dotIndex);
