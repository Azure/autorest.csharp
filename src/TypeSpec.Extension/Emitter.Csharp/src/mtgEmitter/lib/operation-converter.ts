...
export function fromSdkServiceMethod(
    ...
    switch (methodKind) {
        ...
        case "lro":
            ...
            lroServiceMethod.lroMetadata = loadLongRunningMetadata(
                sdkContext,
                sdkMethod
            );
        ...
        case "lropaging":
            ...
            lroPagingMethod.lroMetadata = loadLongRunningMetadata(
                sdkContext,
                sdkMethod
            );
...
export function fromSdkServiceMethodOperation(
    ...
    let generateConvenience = shouldGenerateConvenient(
        sdkContext,
    ...
    if (method.operation.verb === "patch" && generateConvenience) {
        sdkContext.logger.reportDiagnostic({
            ...
            format: {
                methodCrossLanguageDefinitionId:
    ...
    operation = {
        ...
        externalDocsUrl: getExternalDocs(
            sdkContext,
            method.operation.__raw.operation
        )?.url,
        requestMediaTypes: getRequestMediaTypes(method.operation),
        bufferResponse: true,
        generateProtocolMethod: shouldGenerateProtocol(
            sdkContext,
            method.operation.__raw.operation
        ),
    ...
    sdkContext.__typeCache.updateSdkOperationReferences(
        method.operation,
        operation
    );
...
function updateMethodParameter(
    ...
    methodParameter.kind = getParameterKind(
        operationHttpParameter,
...
function fromSdkServiceMethodResponse(
    ...
    return {
        type: methodResponse.type
            ? fromSdkType(sdkContext, methodResponse.type)
            : undefined,
...
export function fromParameter(
    ...
    let retVar = sdkContext.__typeCache.operationParameters.get(p);
    if (retVar) {
    ...
    retVar = {
        name: p.name,
        nameInRequest: getNameInRequest(p),
        summary: p.summary,
        doc: p.doc,
        type: parameterType,
        location: getParameterLocation(p),
        isApiVersion:
            p.name.toLocaleLowerCase() === "apiversion" ||
            p.name.toLocaleLowerCase() === "api-version", // TODO -- we should use `isApiVersionParam` instead
...
export function fromMethodParameter(
    ...
    retVar = {
        ...
        isEndpoint: false,
        explode: false,
        arraySerializationDelimiter: undefined,
        isRequired: !p.optional,
...
function loadLongRunningMetadata(
    ...
    return {
        finalStateVia: convertLroFinalStateVia(
...
export function fromSdkHttpOperationResponse(
    ...
    retVar = {
        ...
        bodyType: sdkResponse.type
            ? fromSdkType(sdkContext, sdkResponse.type)
            : undefined,
        headers: fromSdkServiceResponseHeaders(sdkContext, sdkResponse.headers),
        isErrorResponse:
            sdkResponse.type !== undefined &&
            isErrorModel(sdkContext.program, sdkResponse.type.__raw!),
...
function getMediaTypes(type: SdkType): string[] {
    if (type.kind === "constant") {
        ...
        for (const unionItem of type.variantTypes) {
            if (
                unionItem.kind === "constant" &&
                unionItem.valueType.kind === "string"
            ) {
...
function loadPagingServiceMetadata(
    ...
    if (method.pagingMetadata.nextLinkSegments) {
        nextLink = {
            responseSegments: method.pagingMetadata.nextLinkSegments.map(
                (segment) => getResponseSegmentName(segment)
            ),
        ...
        if (
            ...
            method.pagingMetadata.nextLinkReInjectedParametersSegments.length >
                0
        ) {
            const nextLinkReInjectedParameters = [];
            for (const parameterSegments of method.pagingMetadata
                ...
                if (parameterSegments?.length > 0) {
                    ...
                    const operationParameter = getHttpOperationParameter(
                        method,
                        lastParameterSegment
                    );
    ...
    if (
        ...
        const lastParameterSegment = method.pagingMetadata
        ...
        continuationToken = {
            ...
            responseSegments:
                method.pagingMetadata.continuationTokenResponseSegments!.map(
                    (segment) => getResponseSegmentName(segment)
                ),
            responseLocation: getResponseLocation(
                context,
    ...
    return {
        ...
        itemPropertySegments: method.response.resultSegments!.map(
            (s) => s.name
        ),
        nextLink: nextLink,
...
function getResponseSegmentName(
    segment: SdkServiceResponseHeader | SdkModelPropertyType
): string {
...
function getOperationGroupName(
    ...
    const explicitOperationId = getOperationId(
        context,
...
function getCollectionFormat(
    ...
    return p.kind === "header" || p.kind === "query"
        ? p.collectionFormat
        : undefined;
}
...
function getSerializedName(p: SdkHttpParameter | SdkModelPropertyType): string {
    ...
    return "serializedName" in p && p.kind !== "body"
...
function getNameInRequest(p: SdkHttpParameter | SdkModelPropertyType): string {
    ...
    return p.kind === "header"
        ? normalizeHeaderName(serializedName)
        : serializedName;
}
