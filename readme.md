...
### Debugging projects inside the `autorest.csharp` repo

There are two entry points for debugging the generator:

- Opening `AutoRest.CSharp.sln` in Visual Studio will contain a debugging profile for each sample and test. These only run the "back half", using the yaml file generated by `autorest.modelerfour` as input.
...
### Debugging projects outside the `autorest.csharp` repo
...
To debug `autorest.csharp` in a project that takes TypeSpec files as input, you need to add
```
--option @azure-tools/typespec-csharp.csharpGeneratorPath=/absolute/path/to/AutoRest.CSharp.dll --option @azure-tools/typespec-csharp.debug=true
```
to attach a debugger to the plugin process.

Usually you should find the `AutoRest.CSharp.dll` file here: `[RepoRoot]/artifacts/bin/AutoRest.CSharp/Debug/net6.0/AutoRest.CSharp.dll`.

If you are trying to debug a TypeSpec project inside `azure-sdk-for-net`, you could run
```powershell
dotnet build /t:GenerateCode /p:typespecAdditionalOptions="csharpGeneratorPath=/absolute/path/to/artifacts/bin/AutoRest.CSharp/Debug/net6.0/AutoRest.CSharp.dll%3Bdebug=true"
```
where we have to escape the semicolon `;` in the path to `%3B`. This will also attach a debugger to the plugin process.
