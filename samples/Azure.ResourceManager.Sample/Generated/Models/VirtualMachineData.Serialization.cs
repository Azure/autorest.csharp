...
{
    public partial class VirtualMachineData : IUtf8JsonWriteable, IJsonModel<VirtualMachineData>
    {
        void IUtf8JsonWriteable.Write(Utf8JsonWriter writer) => ((IJsonModel<VirtualMachineData>)this).Write(writer, new ModelReaderWriterOptions("W"));
        ...
        internal static VirtualMachineData DeserializeVirtualMachineData(JsonElement element, ModelReaderWriterOptions options = null)
        {
            ...
            return new VirtualMachineData(id, name, type, systemData.Value, OptionalProperty.ToDictionary(tags), location, plan.Value, OptionalProperty.ToList(resources), identity, OptionalProperty.ToList(zones), hardwareProfile.Value, storageProfile.Value, additionalCapabilities.Value, osProfile.Value, networkProfile.Value, securityProfile.Value, diagnosticsProfile.Value, availabilitySet, virtualMachineScaleSet, proximityPlacementGroup, OptionalProperty.ToNullable(priority), OptionalProperty.ToNullable(evictionPolicy), billingProfile.Value, host, hostGroup, provisioningState.Value, instanceView.Value, licenseType.Value, vmId.Value, extensionsTimeBudget.Value, serializedAdditionalRawData);
        }
