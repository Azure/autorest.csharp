// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using Azure.ResourceManager.Core;

namespace Azure.ResourceManager.MachineLearning
{
    /// <summary> A class representing the WorkspaceConnection data model. </summary>
    public partial class WorkspaceConnectionData : Resource<ResourceGroupResourceIdentifier>
    {
        /// <summary> Initializes a new instance of WorkspaceConnectionData. </summary>
        internal WorkspaceConnectionData()
        {
        }

        /// <summary> Initializes a new instance of WorkspaceConnectionData. </summary>
        /// <param name="id"> The id. </param>
        /// <param name="name"> The name. </param>
        /// <param name="type"> The type. </param>
        /// <param name="category"> Category of the workspace connection. </param>
        /// <param name="target"> Target of the workspace connection. </param>
        /// <param name="authType"> Authorization type of the workspace connection. </param>
        /// <param name="value"> Value details of the workspace connection. </param>
        /// <param name="valueFormat"> format for the workspace connection value. </param>
        internal WorkspaceConnectionData(ResourceGroupResourceIdentifier id, string name, ResourceType type, string category, string target, string authType, string value, ValueFormat? valueFormat) : base(id, name, type)
        {
            Category = category;
            Target = target;
            AuthType = authType;
            Value = value;
            ValueFormat = valueFormat;
        }

        /// <summary> Category of the workspace connection. </summary>
        public string Category { get; }
        /// <summary> Target of the workspace connection. </summary>
        public string Target { get; }
        /// <summary> Authorization type of the workspace connection. </summary>
        public string AuthType { get; }
        /// <summary> Value details of the workspace connection. </summary>
        public string Value { get; }
        /// <summary> format for the workspace connection value. </summary>
        public ValueFormat? ValueFormat { get; }
    }
}
