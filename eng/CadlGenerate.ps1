...
foreach ($directory in Get-ChildItem $testEmitterPath -Directory)
{
    ...
    node node_modules/@typespec/compiler/dist/core/cli/cli.js compile $projectPath/$testName.cadl --emit @azure-tools/typespec-csharp --option @azure-tools/typespec-csharp.emitter-output-dir=$projectPath --option @azure-tools/typespec-csharp.skipSDKGeneration=true --option @azure-tools/typespec-csharp.save-inputs=true
    if (!$?) {
        Pop-Location
